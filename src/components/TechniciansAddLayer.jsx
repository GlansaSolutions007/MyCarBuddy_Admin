...
setIsSubmitting(true);
  try {
    ...
    if (isEditing) {
      await axios.put(`${API_BASE}TechniciansDetails/UpdateTechnicians`, form, config);
    } else {
      await axios.post(`${API_BASE}TechniciansDetails/InsertTechnicians`, form, config);
    }
    ...
    Swal.fire("Error", err?.response?.data?.message || "Failed to save technician", "error");
  } finally {
...
const handleDistributorChange = (selected) => {
        ...
        <form onSubmit={handleSubmit} encType="multipart/form-data" className="row g-3">
          <div className='row'>
            ...
            <div className='col-sm-6 mt-2'>
              <label className='form-label text-sm fw-semibold text-primary-light mb-8'>Full Name <span className='text-danger'>*</span></label>
              <input
                type='text'
                name='FullName'
            ...
            <div className='col-sm-6 mt-2'>
              <label className='form-label text-sm fw-semibold text-primary-light mb-8'>Email <span className='text-danger'>*</span></label>
              <input
                type='email'
                name='Email'
                className='form-control radius-8'
            ...
            <div className='col-sm-6 mt-2'>
              <label className='form-label text-sm fw-semibold text-primary-light mb-8'>Phone Number <span className='text-danger'>*</span></label>
              <input
                type='text'
                name='PhoneNumber'
                className='form-control radius-8'
  ...
  <FormError error={errors.SkillID} />
            ...
            <div className='col-sm-6 mt-2'>
  <label className='form-label text-sm fw-semibold text-primary-light mb-8'>Distributor <span className='text-danger'>*</span></label>
  <Select
    name='DistributorID'
    options={distributors.map((d) => ({ value: d.DistributorID, label: d.FullName }))}
    value={
      formData.DistributorID
        ? {
            value: formData.DistributorID,
            label: distributors.find((d) => d.DistributorID === formData.DistributorID)?.FullName,
          }
        : null
    }
...
<div className='col-sm-6 mt-2'>
  <label className='form-label text-sm fw-semibold text-primary-light mb-8'>Dealer <span className='text-danger'>*</span></label>
  <Select
    name='DealerID'
    options={dealers.map((d) => ({ value: d.DealerID, label: d.FullName }))}
    value={
      formData.DealerID
        ? {
            value: formData.DealerID,
            label: dealers.find((d) => d.DealerID === formData.DealerID)?.FullName,
          }
        : null
    }
    onChange={(selected) => handleChange({ target: { name: "DealerID", value: selected?.value || "" } })}
    classNamePrefix="react-select"
  />
  <FormError error={errors.DealerID} />
            ...
            <div className='col-sm-6 mt-2'>
              <label className='form-label text-sm fw-semibold text-primary-light mb-8'>Password <span className='text-danger'>*</span></label>
            ...
            <div className='col-sm-6 mt-2'>
              <label className='form-label text-sm fw-semibold text-primary-light mb-8'>Confirm Password <span className='text-danger'>*</span></label>
              <input
                type={showPassword ? "text" : "password"}
            ...
            <div className='col-sm-6 mt-2'>
              <label className='form-label text-sm fw-semibold text-primary-light mb-8'>State <span className='text-danger'>*</span></label>
              <Select
                name='StateID'
            ...
            <div className='col-sm-6 mt-2'>
              <label className='form-label text-sm fw-semibold text-primary-light mb-8'>City <span className='text-danger'>*</span></label>
              <Select
                name='CityID'
                options={cities
            ...
            <div className='col-sm-6 mt-2'>
              <label className='form-label text-sm fw-semibold text-primary-light mb-8'>Address Line 1 <span className='text-danger'>*</span></label>
              <textarea
                rows='2'
                name='AddressLine1'
                className='form-control radius-8'
            ...
            <div className='col-sm-6 mt-2'>
              <label className='form-label text-sm fw-semibold text-primary-light mb-8'>Pincode <span className='text-danger'>*</span></label>
              <input
                type='text'
                name='Pincode'
                className='form-control radius-8'
    ...
    <div className="row mb-2 align-items-center" key={index}>
      ...
      <div className="col-md-4">
        <input
          ...
          onChange={(e) =>
            handleDocumentUpload(index, doc.type, e.target.files[0])
          }
        />
        {doc.fileName && (
      ...
      <div className="col-md-3">
        {doc.fileName &&
          (doc.fileName.toLowerCase().endsWith(".pdf") ? (
            <Icon icon="mdi:file-pdf-box" width={32} className="text-danger" />
          ) : (
            <img
        ...
        {selectedTypeLabel && (
          <div className="mt-1 text-muted text-xs">{selectedTypeLabel}</div>
        )}
      </div>
          ...
          Add Document
            ...
            <div className='d-flex justify-content-center gap-3 mt-24'>
              <button type='submit' className='btn btn-primary-600 radius-8 px-14 py-6 text-sm' disabled={isSubmitting}>
                {isSubmitting ? "Saving..." : isEditing ? "Update" : "Save"} Technician
              </button>
            </div>
          </div>
        </form>
