...
const BookingReportSortingLayer = () => {
    ...
    const statusFromReports = queryParams.get("status");
    const fromDateFromReports = queryParams.get("from");
    const toDateFromReports = queryParams.get("to");

    useEffect(() => {
        if (statusFromReports) setStatus(statusFromReports);
        if (fromDateFromReports) setStartDate(fromDateFromReports);
        if (toDateFromReports) setEndDate(toDateFromReports);
    }, []);


    useEffect(() => {
        if (statusFromReports) {
            setStatus(statusFromReports.toLowerCase());
        }
    }, [statusFromReports]);
...
export default BookingReportSortingLayer;
