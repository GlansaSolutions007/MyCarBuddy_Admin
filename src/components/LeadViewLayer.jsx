...
const LeadViewLayer = () => {
  ...
  useEffect(() => {
    if (lead) {
      ...
      if (lead.VehiclesDetails && lead.VehiclesDetails.length > 0) {
        ...
        setCarBrand(vehicle.BrandID && vehicle.BrandName ? { value: vehicle.BrandID, label: vehicle.BrandName } : null);
        setCarModel(vehicle.ModelID && vehicle.ModelName ? { value: vehicle.ModelID, label: vehicle.ModelName } : null);
        setCarFuelType(vehicle.FuelTypeID && vehicle.FuelTypeName ? { value: vehicle.FuelTypeID, label: vehicle.FuelTypeName } : null);
 ...
 // Fetch brands from API
  ...
  const fetchModels = async () => {
    try {
      const res = await axios.get(`${API_BASE}VehicleModels/GetListVehicleModel`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const handleSubmitCarDetails = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE}Leads/InsertOrUpdateFacebookLead`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
                  ...
                  <div className="p-3 border radius-16 bg-light">
                    <div className="row g-3">
                      ...
                      <div className="col-md-6">
                        ...
                        <Select
                           options={fuelTypes.map(fuelType => ({ value: fuelType.FuelTypeID, label: fuelType.FuelTypeName }))}
                          value={carFuelType}
                      ...
                      <div className="col-md-6">
                        ...
                        <Select
                          options={brands.map(brand => ({ value: brand.BrandID, label: brand.BrandName }))}
                          value={carBrand}
                      ...
                      <div className="col-md-6">
                       ...
                       <div className="col-md-6">
                        ...
                        <input
                          ...
                          onChange={(e) => setCarYearOfPurchase(e.target.value)}
                        />
                      </div>
                    </div>
                    <div className="d-flex justify-content-end mt-3 gap-10">
                    ...
                    >
                      ...
                      <div>
                        <div className="text-sm text-secondary-light">
                          <strong>Discussion Notes: </strong>{item.Notes || "-"}
                        </div>
                        {item.NextAction && (
                          <div className="text-sm text-secondary-light">
                        ...
                        <div className="text-sm text-secondary-light">
                          <strong>Updated By: </strong>{item.Created_By || "-"}
                        </div>
                      </div>
                    </li>
                  ))}
