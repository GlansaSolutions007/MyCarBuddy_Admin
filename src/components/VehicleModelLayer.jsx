				...
				<div className="card h-100">
					<div className="card-body">
						<form onSubmit={handleSubmit} className="form" noValidate>
							...
							<div className="mb-3">
								<label htmlFor="format" className="text-sm fw-semibold text-primary-light mb-8">
							...
							<div className="mb-3">
								...
								<Select
									...
									value={
										brand.find((b) => b.BrandID === formData.BrandID)
											? { value: formData.BrandID, label: brand.find((b) => b.BrandID === formData.BrandID)?.BrandName }
											: null
									}
							...
							<div className="mb-3">
								...
								<Select
									...
									value={fuelTypeOptions.find((option) => String(option.value) === String(formData.FuelTypeID)) || null}
									onChange={(selected) => setFormData({ ...formData, FuelTypeID: selected?.value || "" })}
									classNamePrefix="react-select"
							...
							<div className="mb-3">
								...
								<Select
									...
									value={vehicleCategoryOptions.find((option) => option.value === formData.VehicleCategory) || null}
									onChange={(selected) => setFormData({ ...formData, VehicleCategory: selected?.value || "" })}
									classNamePrefix="react-select"
									className={errors.VehicleCategory ? "is-invalid" : ""}
									placeholder="-- Select Category --"
								/>
								<FormError error={errors.VehicleCategory} />
							</div>

							<button type="submit" className="btn btn-primary-600 radius-8 px-14 py-6 text-sm" disabled={isSubmitting}>
								{isSubmitting ? "Saving..." : formData.ModelID ? "Update Model" : "Add Model"}
							</button>
						</form>
					</div>
				</div>
						...
						<Select
							...
							value={
								filterBrandID
									? {
											value: filterBrandID,
											label: brand.find((s) => s.BrandID === filterBrandID)?.BrandName || "Selected Brand",
									  }
									: { value: "", label: "All Brands" }
							}
